...
import com.my1stle.customer.portal.web.controller.scheduling.EventController;
import com.my1stle.customer.portal.web.controller.scheduling.SuggestionController;
import com.my1stle.customer.portal.web.exception.UserRegistrationInstallationNotFoundException;
...
@ControllerAdvice( assignableTypes = {RegistrationController.class }, annotations = {RestController.class})
public class RestResponseEntityExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    // 404
    @ExceptionHandler({ UserRegistrationInstallationNotFoundException.class })
    public ResponseEntity<Object> handleUserInstallationNotFound(final RuntimeException ex, final WebRequest request) {
        logger.error("404 Status Code", ex);
        final GenericResponse bodyOfResponse = new GenericResponse("Unable to find an installation for that username/email. Please enter a different email", "UserInstallationNotFound");
        return handleExceptionInternal(ex, bodyOfResponse, new HttpHeaders(), HttpStatus.NOT_FOUND, request);
    }
