;(function ($, window, document, undefined) {
    ...
    $.quiz = function (el, options) {
        ...
        var questions = base.options.questions,
            ...
            submitButtonText = base.options.submitButtonText,
            currentQuestion = 1,
            score = 0,
            answerLocked = false;
        ...
        base.methods = {
            init: function () {
                ...
                $(document).on('click', '#quiz-submit-btn', function (e) {
                    e.preventDefault();
                    base.methods.submit();
                });
            ...
            setup: function () {
                ...
                quizHtml += '<div id="quiz-incorrect-response" class="alert alert-danger alert-dismissible alert-outline-coloured" style="display:none" role="alert">\n' +
                    '<button type="button" class="close" data-dismiss="alert" style="display:none" aria-label="Close" style="display: none">' +
                    '<span aria-hidden="true"></span>' +
                    '</button>' +
                    '<div id="quiz-incorrect-response-message" class="alert-message">' +
                    '</div>' +
                    '</div>';
                quizHtml += '<div id="quiz-controls">';
                quizHtml += '<p id="quiz-response"></p>';
                quizHtml += '<div id="quiz-buttons">';
                quizHtml += '<a href="#" id="quiz-next-btn" class="btn-primary">' + nextButtonText + '</a>';
                quizHtml += '<a href="#" id="quiz-finish-btn" class="btn-primary">' + finishButtonText + '</a>';
                quizHtml += '<a href="#" id="quiz-restart-btn" class="btn-primary">' + restartButtonText + '</a>';
                quizHtml += '<a href="#" id="quiz-submit-btn" class="btn-primary">' + submitButtonText + '</a>';
                quizHtml += '</div>';
                quizHtml += '</div>';
            ...
            answerQuestion: function (answerEl) {
                ...
                if (selected === correct) {
                    answerLocked = true;
                    $('#quiz-incorrect-response').hide();
                    $('a.incorrect').removeClass('incorrect');
                    ...
                    $('#quiz-incorrect-response').show();
                    response = questions[currentQuestionIndex].incorrectResponse;
                    $('#quiz-incorrect-response-message').html(response);
